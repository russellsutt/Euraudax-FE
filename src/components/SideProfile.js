...
class SideProfile extends Component {
    ...
    renderHostedEvents = () => {
        if (this.props.user.events && this.props.user.events.length > 0 ) {
            return <Container fluid><ListGroup> {this.props.user.events.map(event => <ListGroup.Item key={event.id} variant="info" action onClick={() => { this.props.renderEvent(event.id) }}>{event.title}</ListGroup.Item>)} </ListGroup></Container>
        } else {
    ...
    renderAttendingEvents = () => {
        if (this.props.attending.length > 0 ) {
            return  this.props.attending.map(attendee => <ListGroup.Item key={attendee.id} variant="info" action onClick={() => { this.props.renderEvent(attendee.event.id) }}>{attendee.event.title}</ListGroup.Item>)
        } else {
